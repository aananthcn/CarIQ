...
# Define paths
ROOTFS_UPDATE="/update/downloads/cariq-en1-image-raspberrypi5.rootfs.tar.bz2"
...
if [ -f "$ROOTFS_UPDATE" ]; then
    ...
    chroot "$NEW_ROOT" /bin/sh <<'EOF'
        ...
        pv "/downloads/cariq-en1-image-raspberrypi5.rootfs.tar.bz2" | tar --exclude='var/*' --exclude='./boot/*' -xjf - -C /mnt || {
